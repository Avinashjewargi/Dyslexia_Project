...
// Flex layout • i18n • animated LexiAI
...
function DDrop({ trigger, children }) {
  ...
  return (
    <div style={S.ddWrap} onMouseEnter={() => setO(true)} onMouseLeave={() => setO(false)}>
      <button type="button" style={{ ...S.link, ...(o ? S.linkHov : {}) }}>
        {trigger}
    ...
    <>
      ...
      <nav style={S.nav}>
        ...
        <div style={S.right}>
          ...
          <button type="button" style={S.btnOutline} className="nb-btn-outline" onClick={onOpenSettings}>
            <SettingsIcon size={14}/>
            {t('navbar.accessibility','Accessibility')}
          </button>

          <button type="button" className="nb-mob-tog" style={S.mobTog} onClick={() => setMob(v=>!v)} aria-label="Menu">
      ...
      {mob && (
        <div style={S.mobMenu}>
          ...
          {user ? (
            <button type="button" style={{...S.link,color:'#f87171',justifyContent:'flex-start'}} onClick={logout}>
              <LogOut size={14}/> {t('navbar.logout','Logout')}
            </button>
          ) : (
