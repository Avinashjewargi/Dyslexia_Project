...
export default function Chatbot() {
  ...
  const sendMessage = async () => {
    ...
    if (!isOnline) {
      try {
        ...
        setChat((prev) => [...prev, { sender: "Bot", text: "Oops! Something went wrong in offline mode.", type: 'text', timestamp: new Date() }]);
        setIsLoading(false);
        setIsGenerating(false);
        return;
      }
    ...
    try {
      ...
      if (error.name === 'CanceledError') {
        setChat((prev) => [...prev, { sender: "Bot", text: "Generation stopped.", type: 'text', timestamp: new Date() }]);
      } else {
        const errorMsg = "Oops! Something went wrong. Try again!";
        setChat((prev) => [...prev, { sender: "Bot", text: errorMsg, type: 'text', timestamp: new Date() }]);
        speak(errorMsg);
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <>
      <style>
        {`
          ...
          * {
          ...
          /* ✅ FIXED: Sticky bottom-right on scroll */
          #chatbot-root {
      ...
      {/* ✅ Calculator Popup - Fixed position */}
      {showCalculator && (
        <div className="calculator-popup-wrapper">
          <div style={{
            background: "white",
            borderRadius: "20px",
            padding: "20px",
            ...
            <div style={{ textAlign: 'center' }}>
              {[['7','8','9','+'],['4','5','6','-'],['1','2','3','*'],['0','.','=','/']].map((row, ri) => (
                <div key={ri} style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
                  {row.map(btn => (
                    <button
                      key={btn}
      ...
      {/* ✅ Floating Action Button - Always fixed bottom-right */}
      <div className="chatbot-fab-wrapper">
        ...
        >
          <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.2))", position: 'relative', zIndex: 2 }}>
          ...
          <span style={{
            ...
            boxShadow: isOnline ? "0 2px 8px rgba(16, 185, 129, 0.4)" : "0 2px 8px rgba(239, 68, 68, 0.4)",
            zIndex: 3,
          }}/>
      ...
      {/* ✅ Chat Window - Fixed bottom-right, above the FAB */}
      {isOpen && (
        <div className="chatbot-window-wrapper" style={{ width: chatWidth, height: chatHeight }}>
          <div style={{
            width: "100%",
            height: "100%",
            display: "flex",
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                ...
                <div>
                  <h4 style={{ margin: 0, fontSize: "17px", fontWeight: "600" }}>Dyslexia Assistant</h4>
                  <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "4px" }}>
                    {[{label:'Online', val:true, color:'#10b981'},{label:'Offline', val:false, color:'#ef4444'}].map(({label,val,color}) => (
                      <button key={label} onClick={() => setIsOnline(val)} style={{
                        display: "flex", alignItems: "center", gap: "4px",
                        background: isOnline === val ? `rgba(${val ? '16,185,129' : '239,68,68'}, 0.25)` : "rgba(255,255,255,0.1)",
                        border: `1.5px solid ${isOnline === val ? color : "rgba(255,255,255,0.3)"}`,
                        borderRadius: "12px", padding: "3px 8px",
                        fontSize: "11px", cursor: "pointer", color: "white", fontWeight: "600",
                        transition: "all 0.2s ease",
                      }}>
                        <span style={{ width:"6px", height:"6px", borderRadius:"50%", background: color, boxShadow: isOnline===val ? `0 0 6px ${color}` : "none" }}/>
                        {label}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                {[
                  { title: "New Chat", onClick: clearChat, icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14M5 12h14"/></svg> },
                  { title: "Calculator", onClick: () => setShowCalculator(!showCalculator), active: showCalculator, icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><line x1="8" y1="10" x2="8" y2="10"/><line x1="12" y1="10" x2="12" y2="10"/><line x1="16" y1="10" x2="16" y2="10"/><line x1="8" y1="14" x2="8" y2="14"/><line x1="12" y1="14" x2="12" y2="14"/><line x1="16" y1="14" x2="16" y2="14"/><line x1="8" y1="18" x2="16" y2="18"/></svg> },
                  { title: isExpanded ? "Minimize" : "Expand", onClick: () => setIsExpanded(!isExpanded), icon: isExpanded
                  ...
                  { title: "Close", onClick: () => setIsOpen(false), icon: <span style={{fontSize:'18px'}}>✕</span> },
                ].map(({title, onClick, icon, active}) => (
                  <button key={title} onClick={onClick} title={title} style={{
                    background: active ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.2)",
                  ...
                  onMouseLeave={(e) => e.currentTarget.style.background = active ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.2)"}
                  >
                    {icon}
                  </button>
                ))}
            ...
            <div className="modern-scrollbar" style={{
              ...
              {chat.length === 0 && (
                <div style={{ textAlign: "center", marginTop: "80px" }}>
                  ...
                  <h3 style={{ 
                    fontSize: "24px", fontWeight: "700", margin: "0 0 12px 0",
                    background: "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)",
                    WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text"
                  }}>
                    {isOnline ? "Hello! I'm Your Dyslexia Assistant" : "Hello! I'm in Offline Mode"}
                  </h3>
                  <p style={{ fontSize: "15px", color: "#64748b", lineHeight: "1.6", margin: "0 0 32px 0", maxWidth: "320px", marginLeft: "auto", marginRight: "auto" }}>
                    {isOnline ? "I can help you with reading, OCR, TTS, and answer questions about anything!" : "I can help you learn how to use this dyslexia app!"}
                  </p>
                  {!isOnline && (
                    <div style={{
                      background: "linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)",
                      border: "1px solid #60a5fa", borderRadius: "12px",
                      padding: "12px 16px", margin: "0 auto 20px auto", maxWidth: "360px",
              ...
              {chat.map((msg, index) => (
                <div key={index} className="message-enter" style={{ alignSelf: msg.sender === "User" ? "flex-end" : "flex-start", maxWidth: "85%" }}>
                  <div style={{
                    padding: "14px 18px",
                    borderRadius: msg.sender === "User" ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
                    background: msg.sender === "User" ? "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)" : "white",
                    color: msg.sender === "User" ? "white" : "#1e293b",
                    boxShadow: msg.sender === "User" ? "0 4px 12px rgba(99, 102, 241, 0.25)" : "0 2px 8px rgba(0,0,0,0.08)",
                    wordWrap: "break-word",
                  }}>
                    {msg.sender === "User" ? (
                      <p style={{ margin: 0, fontSize: "15px", lineHeight: "1.6", fontWeight: "500" }}>{msg.text}</p>
                    ) : (
                      <>
                        {msg.title && <div className="title-box">{msg.emoji} {msg.title}</div>}
                        {msg.imageUrl && (
                          <img src={msg.imageUrl} alt={msg.title || 'Image'} className="chat-image" loading="lazy"
                            onError={(e) => { e.target.src = `https://via.placeholder.com/800x600/6366f1/ffffff?text=${encodeURIComponent(msg.title || 'Image')}`; }}
                          />
                        )}
                        {msg.simplifiedText && <p style={{ margin: "12px 0", fontSize: "15px", lineHeight: "1.8", color: "#334155" }}>{msg.simplifiedText}</p>}
                        {msg.calculation && (
                          <div className="calculator-box">
                            <div style={{ fontSize: "24px", fontWeight: "600", color: "#475569", textAlign: "center" }}>{msg.calculation.expression}</div>
                            <div style={{ fontSize: "18px", textAlign: "center", margin: "12px 0", color: "#94a3b8" }}>=</div>
                            <div style={{ fontSize: "36px", fontWeight: "700", color: "#6366f1", textAlign: "center" }}>{msg.calculation.formatted}</div>
                          </div>
                        )}
                        {msg.facts && msg.facts.length > 0 && (
                          <div className="facts-box">
                            <div style={{ fontWeight: "600", marginBottom: "12px", color: "#0ea5e9", fontSize: "14px" }}>💡 Key Facts</div>
                            {msg.facts.map((fact, i) => <div key={i} className="fact-item">{fact}</div>)}
                          </div>
                        )}
                        {!msg.simplifiedText && !msg.facts?.length && !msg.calculation && (
                          <p style={{ margin: 0, fontSize: "15px", lineHeight: "1.7", whiteSpace: "pre-line" }}>{msg.text}</p>
                        )}
                      </>
                    )}
                  </div>
                  <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "6px", textAlign: msg.sender === "User" ? "right" : "left", fontWeight: "500" }}>
              ...
              {isLoading && (
                <div style={{ alignSelf: "flex-start", display: "flex", flexDirection: "column", gap: "12px", width: "100%", maxWidth: "85%" }}>
                  <div style={{
            ...
            {/* Input Area */}
            <div style={{ padding: "20px 24px", background: "white", borderTop: "1px solid #e2e8f0", flexShrink: 0 }}>
              <div style={{
                display: "flex", gap: "12px", background: "#f8fafc",
                borderRadius: "16px", padding: "8px",
                border: "2px solid #e2e8f0", transition: "border-color 0.2s ease",
              }}
              onFocus={(e) => e.currentTarget.style.borderColor = "#6366f1"}
              onBlur={(e) => e.currentTarget.style.borderColor = "#e2e8f0"}
              >
                {isGenerating && (
                  <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", marginLeft: "8px" }}>
                    <svg width="28" height="28" viewBox="0 0 28 28" style={{ animation: "spin 1s linear infinite" }}>
                      ...
                      <circle cx="14" cy="14" r="11" fill="none" stroke="#6366f1" strokeWidth="2.5" strokeDasharray="69" strokeDashoffset="17" strokeLinecap="round"/>
                    </svg>
                    {isOnline && (
                      <button onClick={stopGeneration} title="Stop generating" style={{
                        position: "absolute", width: "18px", height: "18px",
                        background: "#6366f1", border: "none", borderRadius: "3px",
                ...
                <input
                  type="text"
                  value={input}
                  placeholder={isGenerating ? (isOnline ? "Searching..." : "Processing...") : !isOnline ? "Ask me about dyslexia features! (Offline)" : "Ask me anything..."}
                  onChange={(e) => setInput(e.target.value)}
                  onKeyPress={handleKeyPress}
                  disabled={isLoading}
                  style={{
                    flex: 1, padding: "14px 16px", border: "none", borderRadius: "12px",
                    fontSize: "15px", outline: "none", background: "white",
                    color: "#1e293b", fontWeight: "500",
                  }}
                />

                <button onClick={startListening} disabled={isListening || isLoading} style={{
                  padding: "12px",
                  ...
                  fontSize: "20px", minWidth: "48px", height: "48px",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all 0.2s ease", boxShadow: "0 4px 12px rgba(99, 102, 241, 0.25)",
                }}
                ...
                <button onClick={sendMessage} disabled={!input.trim() || isLoading} style={{
                  padding: "12px",
                  background: input.trim() && !isLoading ? "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)" : "#e2e8f0",
                  color: input.trim() && !isLoading ? "white" : "#94a3b8",
                  border: "none", borderRadius: "12px",
                  cursor: input.trim() && !isLoading ? "pointer" : "not-allowed",
                  fontSize: "20px", minWidth: "48px", height: "48px", fontWeight: "600",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all 0.2s ease",
